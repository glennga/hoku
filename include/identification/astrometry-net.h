...
/// // less than 20 degrees of each. The entries stored are the BSC5 ID numbers and the four hash codes generated by
/// // quad.
...
class AstrometryNet {
  ...
  private:
    ...
    /// Alias for a list of catalog ID quads (STL vector of 4-element arrays of integers).
    using label_list_quad = std::vector<std::array<int, 4>>;
  ...
  private:
    ...
    static int insert_astro_h (Nibble &, std::vector<int> &, double, const label_quad &, double);
    
    label_quad query_for_asterism (const label_quad &);
    
    Rotation propose_alignment (const index_quad &, const label_quad &);
    models classify_matches (const label_quad &, const Rotation &);
    
    label_list_quad nearby_asterisms (const label_quad &);
