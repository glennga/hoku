...
{
    for (unsigned int i = 0; i < this->stars.size(); i++) this->stars[i] = Star::reset_hr(this->stars[i]);
    return this->stars;
}
...
int Benchmark::record_current_plot()
{
    ...
    {
        for (const double &component : {s[0], s[1], s[2]}) current_record << component << " ";
        current_record << s.get_hr() << "\n";
    }
    ...
    for (const ErrorModel &model: this->error_models)
    {
        ...
        {
            for (const double &component: {s[0], s[1], s[2]}) error_record << component << " ";
            error_record << s.get_hr() << " " << model.plot_color << "\n";
        }
    }
...
int Benchmark::display_plot()
{
    ...
    if (std::ifstream(this->CURRENT_PLOT.c_str()) || std::ifstream(this->ERROR_PLOT.c_str())) return -1;
