 ...
 * @file: chomp.cpp
 *
 * @brief: Source file for Chomp class, which is a child class of Nibble and provides more
 ...
 */
Nibble::sql_row Chomp::k_vector_query(const std::string &focus, const std::string &fields,
    ...
    s_endpoints = search_table("rowid BETWEEN " + std::to_string((int) j_b) + " AND " +
                                  std::to_string((int) j_t), "k_value", expected / 2);

    select_table(s_table);
    return search_table("rowid BETWEEN " + std::to_string(s_endpoints.front()) + " AND " +
