...
///
/// Source file for the trials. This holds the namespace........
...
/// working benchmark.
///
/// @param b Working Nibble connection and set number.
/// @param log Open stream to log file.
void Trial::record_angle (WorkingBenchmark &b, std::ofstream &log) {
    Angle::Parameters p;
    ...
    for (p.query_sigma = DCANG::QS_MIN; p.query_sigma <= DCANG::QS_MAX; p.query_sigma += DCANG::QS_STEP) {
        for (p.match_sigma = DCANG::MS_MIN; p.match_sigma <= DCANG::MS_MAX; p.match_sigma += DCANG::MS_STEP) {
            for (p.match_minimum = DCANG::MM_MIN; p.match_minimum <= DCANG::MM_MAX; p.match_minimum += DCANG::MM_STEP) {
                ...
                Benchmark input = Benchmark::parse_from_nibble(*b.nb, b.set_n);
                ...
                log << b.set_n << "," << s.size() << "," << results.size() << "," << matches_found << ",";
...
/// @param b Working Nibble connection and set number.
/// @param log Open stream to log file.
void Trial::record_plane (WorkingBenchmark &b, std::ofstream &log) {
    Plane::Parameters p;
    ...
    for (p.quadtree_w = DCPLA::BQT_MIN; p.quadtree_w <= DCPLA::BQT_MAX; p.quadtree_w += DCPLA::BQT_STEP) {
        ...
        for (p.match_minimum = DCPLA::MM_MIN; p.match_minimum <= DCPLA::MM_MAX; p.match_minimum += DCPLA::MM_STEP) {
            record_plane_as_ms_ms(b, log, q_root, p);
        }
    }
}
...
void Trial::record_plane_as_ms_ms (WorkingBenchmark &b, std::ofstream &log, std::shared_ptr<QuadNode> &q_root,
                                   Plane::Parameters &p) {
    Star::list s;
    double fov;
...
/// @param b Working Nibble connection and set number.
/// @param log Open stream to log file.
void Trial::record_sphere (WorkingBenchmark &b, std::ofstream &log) {
    Sphere::Parameters p;
    ...
    for (p.quadtree_w = DCSPH::BQT_MIN; p.quadtree_w <= DCSPH::BQT_MAX; p.quadtree_w += DCSPH::BQT_STEP) {
        ...
        for (p.match_minimum = DCSPH::MM_MIN; p.match_minimum <= DCSPH::MM_MAX; p.match_minimum += DCSPH::MM_STEP) {
            record_sphere_as_ms_ms(b, log, q_root, p);
        }
    }
}
...
void Trial::record_sphere_as_ms_ms (WorkingBenchmark &b, std::ofstream &log, std::shared_ptr<QuadNode> &q_root,
                                    Sphere::Parameters &p) {
    Star::list s;
    double fov;
...
/// @param b Working Nibble connection and set number.
/// @param log Open stream to log file.
void Trial::record_astro (WorkingBenchmark &b, std::ofstream &log) {
    Astro::Parameters p;
    ...
    (*b.nb).select_table(p.center_name);
    unsigned int n = (unsigned) (*b.nb).search_table("MAX(rowid)", 1)[0];
    Nibble::tuple asterisms = (*b.nb).search_table("i, j, k", n * 3);
    
    // Convert ASTRO_C table into stars.
    astro_stars.reserve(n / 3);
    for (unsigned int i = 0; i < n; i += 3) {
        Nibble::tuple astro_i = (*b.nb).table_results_at(asterisms, 3, i);
    ...
    for (p.kd_tree_w = DCAST::BKT_MIN; p.kd_tree_w <= DCAST::BKT_MAX; p.kd_tree_w += DCAST::BKT_STEP) {
        std::shared_ptr<KdNode> star_root = std::make_shared<
            KdNode>(KdNode::load_tree((*b.nb).all_bsc5_stars(), p.kd_tree_w));
            ...
            for (p.u_tn = DCAST::UT_MIN; p.u_tn <= DCAST::UT_MAX; p.u_tn += DCAST::UT_STEP) {
                for (p.u_fp = DCAST::UT_MIN; p.u_fp <= DCAST::UT_MAX; p.u_fp += DCAST::UT_STEP) {
                    for (p.u_fn = DCAST::UT_MIN; p.u_fn <= DCAST::UT_MAX; p.u_fn += DCAST::UT_STEP) {
                        record_astro_ka_qs_ms(b, log, star_root, astro_root, p);
                    }
                }
            }
        }
...
void Trial::record_astro_ka_qs_ms (WorkingBenchmark &b, std::ofstream &log, std::shared_ptr<KdNode> &star_root,
                                   std::shared_ptr<KdNode> &astro_root, Astro::Parameters &p) {
    Star::list s;
    double fov;
...
/// @param b Working Nibble connection and set number.
/// @param log Open stream to log file.
void Trial::record_pyramid (WorkingBenchmark &b, std::ofstream &log) {
