...
/// @param s Reference to the star list we are to store our results in.
...
void Experiment::Query::generate_n_stars (Chomp &ch, const unsigned int n, Star::list &s, Star &center,
                                          const double fov) {
...
double Experiment::Reduction::percentage_correct (const Star::list &big_c, const Identification::stars_either &r) {
    double percentage = 0;
    ...
    std::for_each(r.result.begin(), r.result.end(), [&percentage, &big_c] (const Star &r_i) -> void {
        percentage += (std::find_if(big_c.begin(), big_c.end(), [&r_i] (const Star &c) -> bool {
            return Star::within_angle(c, r_i, std::numeric_limits<double>::epsilon()) &&
                   c.get_label() == r_i.get_label();
        }) != big_c.end()) ? 1.0 : 0;
    });

    return percentage / r.result.size();
}
...
double Experiment::Map::percentage_correct (const Star::list &big_i, const Identification::stars_either &b,
                                            const double fov) {
    double count = 0;
