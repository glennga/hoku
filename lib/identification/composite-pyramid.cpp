...
Composite::trios_either Composite::find_catalog_stars (const Star::trio &b_f) {
    ...
    if (big_a[0].size() == big_a[1].size() == big_a[2].size() == big_a[3].size() == big_a[4].size()
        ...
        return trios_either{Star::trio{max_a[0], max_a[1], max_a[2]}, 0};
    }
}
...
/// by the angle method.
Composite::stars_either Composite::reduce () {
        ...
        for (unsigned int dk = 1; dk < big_i->size() - dj - 1; dk++) {
            for (unsigned int di = 0; di < big_i->size() - dj - dk - 1; di++) {
                ...
                labels_list_list big_r_ell = this->query_for_trios(
                        Trio::planar_area((*big_i)[i], (*big_i)[j], (*big_i)[k]),
                ...
                return stars_either{Star::list{ch.query_hip(big_r_ell[0][0]), ch.query_hip(big_r_ell[0][1]),
                                               ch.query_hip(big_r_ell[0][2])}, 0};
            }
        }
    }
