...
int Astro::insert_astro_h (Nibble &nb, std::vector<int> &a_count, const double a_limit, const label_quad &i,
                           const double fov) {
    Star::list s_l = {nb.query_bsc5(i[0]), nb.query_bsc5(i[1]), nb.query_bsc5(i[2]), nb.query_bsc5(i[3])};
    auto check_hash = [a_count, a_limit, i] (const int &i_label) -> bool {
        return a_count[i[i_label]] < a_limit;
    };
    ...
    if (check_hash(0) && check_hash(1) && check_hash(2) && check_hash(3)) {
        ...
        if (Star::within_angle(s_l, fov) && h_t[0] + h_t[1] + h_t[2] + h_t[3] != 0) {
            ...
            nb.insert_into_table("label_0, label_1, label_2, label_3, cx, cy, dx, dy",
                                 {(double) i[0], (double) i[1], (double) i[2], (double) i[3], h_t[0], h_t[1], h_t[2],
                                     h_t[3]});
            return 0;
        }
    }
...
int Astro::generate_hash_table (const double fov, const int a_limit, const std::string &hash_table) {
    ...
    std::vector<int> a_count(Nibble::BSC5_MAX_ID + 1);
    
    // Insert the relation's attributes.
    nb.create_table(hash_table, "label_0 INT, label_1 INT, label_2 INT, label_3 INT, cx FLOAT, cy FLOAT, dx FLOAT, dy "
        "FLOAT");
    ...
    for (unsigned int i = 0; i < Nibble::BSC5_TABLE_LENGTH - 3; i++) {
        ...
        std::cout << "\r" << "Current *A* Star: " << nb.query_bsc5(i).get_label();
...
int Astro::generate_center_table (const std::string &hash_table, const std::string &center_table) {
    ...
    // Find all possible catalog IDs from the ASTRO_H table.
    Nibble::tuple label = nb.search_table("label_0, label_1, label_2, label_3", n);
    
    // Insert the relation's attributes.
    nb.create_table(center_table, "label_0 INT, label_1 INT, label_2 INT, label_3 INT, i FLOAT, j FLOAT, k FLOAT");
    initial_transaction.commit();
    nb.select_table(center_table);
    
    // For every catalog ID quad, we generate the center and store this.
    for (unsigned int i = 0; i < label.size(); i += 4) {
        SQLite::Transaction transaction(*nb.db);
        Star center = Asterism::center(
            {nb.query_bsc5(i), nb.query_bsc5(i + 1), nb.query_bsc5(i + 2), nb.query_bsc5(i + 3)});
        
        nb.insert_into_table("label_0, label_1, label_2, label_3, i, j, k",
                             {label[i], label[i + 1], label[i + 2], label[i + 3], center[0], center[1], center[2]});
        transaction.commit();
    }
...
/// @return [-1][-1][-1][-1] if no match is found. The catalog IDs of the input stars respectively, otherwise.
Astro::label_quad Astro::query_for_asterism (const index_quad &b_i) {
    ...
    Chomp::tuple matches = ch.k_vector_query("cx", "label_0, label_1, label_2, label_3", h[0] - epsilon, h[0] + epsilon,
    ...
    for (unsigned int i = 0; i < matches.size(); i += 4) {
        ...
        if (within_epsilon(i + 1, h[1]) && within_epsilon(i + 2, h[2]) && within_epsilon(i + 3, h[3])) {
            Astro::label_quad in_given_order = {0, 0, 0, 0};
...
/// Given a set of indices from our input and another of set catalog IDs, determine the rotation to take our inertial
/// stars to the body.
///
/// @param b_i Selected indices from our input to represent the quad.
/// @param r Catalog IDs that represent the asterism.
/// @return Identity quaternion if no asterism is found. An inertial->body rotation otherwise.
Rotation Astro::propose_alignment (const index_quad &b_i, const label_quad &r_labels) {
    auto query_for_label = [this, &r_labels] (const int &label) -> Star {
        return this->ch.query_bsc5(r_labels[label]);
    };
    
    // Return identity if no asterism is found.
    if (r_labels[0] + r_labels[1] + r_labels[2] + r_labels[3] == -4) {
        return Rotation::identity();
    }
    
    // Otherwise, we query for the stars in the inertial frame.
    Asterism::stars r = {query_for_label(0), query_for_label(1), query_for_label(2), query_for_label(3)};
...
/// Given a quad of catalog IDs, and a proposed alignment, determine which nearby stars from the catalog ID quad match 
/// the input and which do not. **We only use the first star in the quad.**
///
/// @param r_labels Catalog IDs that represent the asterism.
/// @param q Rotation to take R frame stars to the B frame.
/// @return Model holding "matched" stars, and another "unmatched" stars.
Astro::models Astro::classify_matches (const label_quad &r_labels, const Rotation &q) {
    models m;
    
    // Search for nearby stars to the first catalog ID.
    Star::list nearby = (*star_root).nearby_stars(ch.query_bsc5(r_labels[0]), fov, parameters.nearby_expected,
                                                  ch.all_bsc5_stars());
    ...
    for (const Star &s: this->input) {
        for (unsigned int i = 0; i < nearby.size(); i++) {
            ...
            if (Star::angle_between(b_prime, s) < 3.0 * parameters.match_sigma) {
                // We insert the matched input star with the proper catalog ID if found.
                m[0].emplace_back(Star(s[0], s[1], s[2], b_prime.get_label()));
...
/// Given an quad of stars (represented through catalog IDs, R frame stars), find other nearby asterisms. Return these
/// catalog IDs as a list.
///
/// @param r_labels Catalog IDs that represent the asterism.
/// @return List with r_labels if this is not a valid asterism. Otherwise, the catalog IDs of the nearby asterisms.
Astro::label_list_quad Astro::nearby_asterisms (const label_quad &r_labels) {
    label_list_quad nearby_quad;
    auto query_for_labels = [this, &r_labels] (const int &label) -> Star {
        return this->ch.query_bsc5(r_labels[label]);
    };
    ...
    Star center = Asterism::center({query_for_labels(0), query_for_labels(1), query_for_labels(2), query_for_labels(3)});
    ...
    // Parse all of quad's catalog IDs.
    nearby_quad.reserve(nearby.size());
    ch.select_table(this->parameters.center_name);
    for (const Star &n : nearby) {
        Chomp::tuple n_quad = ch.k_vector_query("i", "label_0, label_1, label_2, label_3",
                                                n[0] - std::numeric_limits<double>::epsilon(),
                                                n[0] + std::numeric_limits<double>::epsilon(), 4);
        
        nearby_quad.push_back(label_quad {(int) n_quad[0], (int) n_quad[1], (int) n_quad[2], (int) n_quad[3]});
    }
...
Star::list Astro::identify (const Benchmark &input, const Parameters &parameters, unsigned int &z,
    ...
    for (int i = 0, j = 1; i < INPUT_SIZE - 3; j = (j < INPUT_SIZE - 3) ? j + 1 : i++ + 2) {
        for (int k = j + 1, m = k + 1; k < INPUT_SIZE - 1; m = (m < INPUT_SIZE - 1) ? m + 1 : k++ + 2) {
            label_quad r_labels = a.query_for_asterism({i, j, k, m});
            unsigned long b_f = 1;
            z++;
            
            // Propose an alignment (r -> b frame). If no asterism can be generated, we break early.
            Rotation a_p = a.propose_alignment({i, j, k, m}, r_labels);
            if (a_p == Rotation::identity()) {
                break;
            }
            
            // Find stars who align from the R frame to a B frame star.
            models bf_models = a.classify_matches(r_labels, a_p);
            
            // Find nearby asterisms. Compute the bayes factor based on these alignments.
            label_list_quad nearby_quad = a.nearby_asterisms(r_labels);
            for (const label_quad &na : nearby_quad) {
