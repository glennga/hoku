...
void Asterism::verify_ab_stars () {
    ...
    auto translate_positive = [] (Mercator &m) -> Mercator {
        return Mercator(m[0] + 0.5, m[1] + 0.5, 1, m.get_hr());
    };
    ...
    if (closest.get_hr() != a.get_hr() && closest.get_hr() != b.get_hr()) {
        auto rotate_positive = [] (Mercator &m) -> Mercator {
            return Mercator(m[1], -m[0] + 1, 1, m.get_hr());
        };
    ...
    if (a[0] > b[0] || a[1] > b[1]) {
        auto reflect_yx = [] (Mercator &m) -> Mercator {
            return Mercator(m[1], m[0], 1, m.get_hr());
        };
...
Asterism::points_cd Asterism::compute_cd_prime () {
    ...
    c_prime = Mercator((c[0] - a[0]) / (b[0] - a[0]), (c[1] - a[1]) / (b[1] - a[1]), 1, c.get_hr());
    d_prime = Mercator((d[0] - a[0]) / (b[0] - a[0]), (d[1] - a[1]) / (b[1] - a[1]), 1, d.get_hr());
