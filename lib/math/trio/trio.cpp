...
double Trio::spherical_area(const Star &b_1, const Star &b_2, const Star &b_3) {
    ...
    double iota = tan(rad * s / 2.0) * tan(rad * (s - ell[0]) / 2.0);
    iota *= tan(rad * (s - ell[1]) / 2.0) * tan(rad * (s - ell[2]) / 2.0);

    // convert back to degrees
    return 4.0 * atan(sqrt(iota)) / rad;
}
 ...
 * distance from star C_1 & star C_3. If focus is equal to <0, 0, 0>, then find the middle fourth of
 * the triangle.
 * 
...
double Trio::recurse_spherical_moment(const Star &c, const int td_n, const int td_i) {
    if (td_n == td_i) {
        ...
        return t_1.recurse_spherical_moment(c, td_n, td_i + 1) +
               t_2.recurse_spherical_moment(c, td_n, td_i + 1) +
               t_3.recurse_spherical_moment(c, td_n, td_i + 1) +
               t_4.recurse_spherical_moment(c, td_n, td_i + 1);
    }
}
...
double Trio::spherical_moment(const Star &b_1, const Star &b_2, const Star &b_3, const int td_h) {
    ...
    // we start at root, where current tree depth is 0
