...
Rotation Rotation::wrap (const Quaternion &q) {
    return {q.data[3], q.data[0], q.data[1], q.data[2]};
}
...
Rotation Rotation::svd (const Star::list &v, const Star::list &w) {
    // TODO: The implementation below with Eigen is way too inefficient for any actual use. Just removing Eigen for now.
    return triad(v, w);
