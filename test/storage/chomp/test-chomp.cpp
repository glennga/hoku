...
/*
 * Verify that the regular query returns correct results. This test is just used to compare
...
void TestChomp::test_regular_query() {
    ...
    kaph = Nibble::search_table(db, "SEP20", "theta BETWEEN 5.004 and 5.005",
                                "star_a_number, star_b_number, theta", 90, 30);

    assert_true(kaph[2] < 5.005 && kaph[2] > 5.004, "RegularQueryFirstResultsWithinBounds");
}

/*
 * Verify that the k-vector query returns the correct results.
 */
