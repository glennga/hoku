...
TEST(Experiment, QuerySetExistence) {
    std::vector<Identification::labels_list> a = {{1, 2, 3},
                                                  {4, 5, 6},
                                                  {6, 7, 8}};
    std::vector<Identification::labels_list> a1 = {{1, 2, 3},
                                                   {4, 5, 6},
                                                   {6, 7, 8}};
    std::vector<Identification::labels_list> a2 = {{1, 2, 3},
                                                   {4, 5, 6},
                                                   {6, 7, 8}};
...
TEST(Experiment, IdentificationTrialAngle) {
    ...
    for (const Nibble::tuple_d b_d : b) {
        ...
        EXPECT_THAT(b_d[4], IsBetweenExperiment(0, 1.0 /
                                                   pow((*cf).GetReal("identification-experiment", "ss-step", 0), 0)));
    ...
    SQLite::Transaction transaction(*(*lu).conn);
    (*(*lu).conn).exec(
            "DELETE FROM IDENTIFICATION WHERE Timestamp = '" + (*lu).timestamp +
            "' AND IdentificationMethod = 'Angle'");
    transaction.commit();
}
