...
TEST(Benchmark, LogCurrentPlotFile) { // NOLINT(cert-err58-cpp,modernize-use-equals-delete)
    ...
    std::remove(std::string(std::string(std::getenv("TEMP")) + "/cuplt.tmp").c_str());
    std::remove(std::string(std::string(std::getenv("TEMP")) + "/errplt.tmp").c_str());
    input.record_current_plot();
    std::ifstream current_plot_from_input(std::string(std::getenv("TEMP")) + "/cuplt.tmp");
...
TEST(Benchmark, LogErrorPlotFile) { // NOLINT(cert-err58-cpp,modernize-use-equals-delete)
    ...
    std::ifstream error_plot_from_input(std::string(std::getenv("TEMP")) + "/errplt.tmp");
