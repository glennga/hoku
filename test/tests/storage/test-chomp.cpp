...
MATCHER_P2(IsBetweenChomp, a, b, // NOLINT(modernize-use-equals-delete)
...
TEST(Chomp, ConstructorNoArguments) { // NOLINT(cert-err58-cpp,modernize-use-equals-delete)
    ...
    EXPECT_FALSE(nb.does_table_exist(bright));
    ...
    EXPECT_TRUE(ch.does_table_exist(bright));
}
...
TEST(Chomp, ConstructorInMemory) { // NOLINT(cert-err58-cpp,modernize-use-equals-delete)
    ...
    EXPECT_FALSE(ch.does_table_exist(hip));
    EXPECT_TRUE(ch.does_table_exist(bright));
...
TEST(Chomp, StarTableComponentsFromLineNoYear) { // NOLINT(cert-err58-cpp,modernize-use-equals-delete)
...
TEST(Chomp, StarTableComponentsFromLine1Year) { // NOLINT(cert-err58-cpp,modernize-use-equals-delete)
...
TEST(Chomp, StarTableYearDifference) { // NOLINT(cert-err58-cpp,modernize-use-equals-delete)
    ...
    EXPECT_THROW(Chomp::year_difference(cf1), std::runtime_error); // NOLINT(cppcoreguidelines-avoid-goto)
...
TEST(Chomp, StarTableTableExistence) { // NOLINT(cert-err58-cpp,modernize-use-equals-delete)
    ...
    EXPECT_TRUE(ch.does_table_exist(ch.bright_table));
    EXPECT_TRUE(ch.does_table_exist(ch.hip_table));
}

/// Check that the angle between NU.03 Canis Majoris and Alpha Canis Majoris (Sirius) are correct.
TEST(Chomp, StarTableCorrectAngleBetweenStars) { // NOLINT(cert-err58-cpp,modernize-use-equals-delete)
    Chomp ch;
...
TEST(Chomp, QueryQuery31700) { // NOLINT(cert-err58-cpp,modernize-use-equals-delete)
    Chomp ch;
...
TEST(Chomp, QueryQuery32349) { // NOLINT(cert-err58-cpp,modernize-use-equals-delete)
    Chomp ch;
...
TEST(Chomp, QueryHip) { // NOLINT(cert-err58-cpp,modernize-use-equals-delete)
...
TEST(Chomp, ListsBrightStarGrab) { // NOLINT(cert-err58-cpp,modernize-use-equals-delete)
    Chomp ch;
...
TEST(Chomp, NearbyBrightStars) { // NOLINT(cert-err58-cpp,modernize-use-equals-delete)
    Chomp ch;
...
TEST(Chomp, NearbyHipStars) { // NOLINT(cert-err58-cpp,modernize-use-equals-delete)
    Chomp ch;
    Star focus = Star::chance();
...
/// query time.
TEST(Chomp, QuerySimpleBound) { // NOLINT(cert-err58-cpp,modernize-use-equals-delete)
    Chomp ch;
...
TEST(Chomp, QuerySimpleBoundMultipleFields) { // NOLINT(cert-err58-cpp,modernize-use-equals-delete)
    Chomp ch;
    ch.select_table("ANGLE_20");
...
TEST(Chomp, KVectorCreate) { // NOLINT(cert-err58-cpp,modernize-use-equals-delete)
...
TEST(Chomp, KVectorQuery) { // NOLINT(cert-err58-cpp,modernize-use-equals-delete)
    // TODO: Finish this test. K-Vector is not being used at the moment, so this is not a huge priority,
}