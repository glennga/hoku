...
\begin{abstract}
    Presented a set of query points within an image coordinate system, constellation queries identify the matching points in a database of known points within a standard coordinate system.
    ...
    This paper studies six existing constellation query processing strategies (Angle, Interior Angle, Spherical Triangle, Planar Triangle, Pyramid, Composite Pyramid) using a unified algorithmic framework and presents an extensive experimental evaluation of the six strategies.
    Given an image with false points, the Pyramid strategy has the highest average accuracy and is the fastest of the six.
    Given an image where each point's true position is distributed randomly (Gaussian noise), the Spherical Triangle strategy's accuracy is the least sensitive to positional change.
\end{abstract}
