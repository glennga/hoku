...
\begin{figure}
    \centering{
        ...
        \caption{
        Depicts the frequency of correct bijections formed with and without the verification step of both the Pyramid
...
\begin{figure*} % HAD TO MOVE THIS GUY TOO...
    ...
    The left plot depicts the average time to obtain $f$, and the right plot depicts the trend line
    $g(f) = A \cdot \mathit{ln}\left( \sigma \right) + B$.
...
Given Gaussian noise, the \Call{DMT}{} process appears to play a larger role with the Angle method and returns to $b$
...
\subsubsection{Which method has the slowest growing $f$ accuracy response to increasing noise?}
...
Relative to the Gaussian noise comparison, the \Call{DMT}{} and $\abs{R} = 1$ criterion play a more equal role in
...
The Pyramid method has the most accurate $f$ on average given images with $\omega$ in~\autoref{eq:numberOfSpikes}
at $99.841 \pm 3.532\%$.
