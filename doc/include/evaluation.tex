...
\begin{table}
    ...
    ($r_b$, such that the correct injection can be formed with $b$) existing in $R$ after retrieval, the number of
    trials where the resulting $R$ meets the $\abs{R} \seq 1$ criterion ($S$), and the average retrieval running time
    ($t_{\AVG}$) given images with no noise.
...
In~\autoref{tab:queryExperimentResults} the average running time (out of $2{,}000$ runs) to obtain an $R$ set is
...
\begin{figure}
    \centering{
        ...
        \caption{
        Depicts the frequency of correct injections $a(h, b)$ formed with and without the verification step of
...
\begin{figure*} % HAD TO MOVE THIS GUY TOO...
    ...
    \end{subfigure}
    \caption{
    Both plots represent some statistic about the resulting injection $h$ produced by each identification strategy
    ...
    $a(h, b) = c \cdot \mathit{\ln}\left( \rho \right) + d$.
