...
Once a unique pair is found, the \Call{DMT}{} procedure requires two additional catalog queries for nearby stars.
\begin{equation}\label{eq:complexityAngle}
    T_{angle} = \left( O\left( \frac{|I|!}{2\left( |I| - 2 \right)!} \right) + 2 \right) T_{query}
\end{equation}
...
\begin{equation}\label{eq:dotComplexity}
    T_{dot} = O\left( \frac{|I|!}{6\left( |I| - 3\right)!}  \right) T_{query}
\end{equation}
...
\Call{Pivot}{} procedure.
This requires $|I|$ additional queries at worst for each combination, giving the numerator in $O$ term.
The last term in $T_{sphere}$ comes from the \Call{DMT}{} call, which requires 6 additional catalog queries for
nearby stars.
\begin{equation}\label{eq:sphereComplexity}
    T_{sphere} = \left(O \left( \frac{|I|! \times |I|}{6 \left( |I| - 3\right)!} \right) + 6 \right) T_{query}
\end{equation}
...
The running time of this algorithm $T_{pyramid}$ is depicted below.
...
Once this is found, an additional three queries are used for the \Call{VerifyP}{} procedure to determine the
pyramid head.
\begin{equation}\label{eq:pyramidComplexity}
    T_{pyramid} = O\left( \frac{6|I|!}{6 \left( |I| - 3\right)!} \right) T_{query}
\end{equation}
...
Once this is found, an additional nine queries are used for the \Call{DMT}{} (six queries) and
\Call{VerifyC}{} (three queries) procedures to determine the alignment and the pyramid head.
The \Call{DMT}{} call is used prior to verification, and may be repeated for all combinations.
\begin{equation}\label{eq:compositeComplexity}
T_{composite} = O\left( \frac{9|I|!}{6 \left( |I| -3 \right)!} \right) T_{query}
\end{equation}
