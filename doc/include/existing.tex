...
Once a unique pair is found, the \Call{DMT}{} procedure requires $|I|$ additional catalog queries at most for nearby
stars.
...
The last term in $T_{sphere}$ comes from the \Call{DMT}{} call, which requires another $|I|$ addtional catalog
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \For{$i \gets 1 \text{\textbf{ to }} |I| - dj - dk$}
        \State $j \gets i + dj$
        \State $k \gets j + dk$
        \\
        \State $b \gets (b_i, b_j, b_k)$
...
Once this is found, an additional four queries are used for the \Call{DMT}{} (one query) and
