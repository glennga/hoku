...
specific feature.
The ideal feature set is one that has a long period of stability, and one with low responses to a deviation change.
...
This deviation is used by the \Call{DMT}{} call, a function used by the Angle, Spherical Triangle, Planar Triangle, and
...
The sensitivity of this hyperparameter selection was characterized in terms of the resulting F-score of the
classification:
\begin{equation}
    \label{eq:sensitivityIdentification}
    \left\lvert\frac{\partial F_1}{\partial \sigma}\right\rvert_{\sigma_o} = \text{response of } F_1 \text{ at }
    \sigma_o \\
\end{equation}
...
\begin{subequations}
    ...
    Where $N$ is defined as the number of samples and $p$ is defined as:
    \[ y =
    \begin{cases}
...
\begin{subequations}
    ...
    \begin{align}
        \text{Reduction hit probability} &= \frac{1}{N} \sum_{i = 0}^N y(i) \\
