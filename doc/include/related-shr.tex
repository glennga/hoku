...
\begin{figure}
    \centering{
    \input{include/floats/generic-identification-shr}
    \caption{
    Flowchart depicting the unified identification framework which all strategies here follow.
    Given a query image, this process returns a map that pairs of subset of image points with points in a reference database.
%    Given a query set $\imageset$, this process returns a function $\solutionmap$ whose domain is some subset of the image $\imagesubset$ and whose codomain is the stellar database, $\databaseset$.
    In the event all image subsets are exhausted, an error is raised and no map is returned (not depicted).
