...
\begin{tikzpicture}[node distance=1.8cm]
    ...
    \node[scale=1](findAlignment)[process, below of=confidentAfterFilter, yshift=-0.5cm]{Find Alignment};
    \node[scale=1](confidentInAlignment)[decision, below of=findAlignment, yshift=-0.5cm] {Confident in $a$?};
    \node[scale=1](returnAlignment)[terminal, right of=confidentInAlignment, xshift = 1.7cm] {Return Alignment};
    ...
    \draw[->, >=stealth] (confidentInAlignment) -- node[xshift=-0.05cm, yshift=0.2cm]{Yes}(returnAlignment);
    \draw[->, >=stealth] (confidentAfterFilter) -- node[anchor=east, yshift=0.1cm]{Yes} (findAlignment);
    \draw[->, >=stealth] (findAlignment) -- node[scale=1.3, xshift=0.5cm, yshift=-0.15cm]{$a$}(confidentInAlignment);
    ...
    |- (pickQueryStars.west);
    \draw[->, >=stealth](confidentInAlignment.west) -- ++(-1.1cm, 0cm) node[anchor=south, xshift=0.5cm]{No}
    |- (pickQueryStars.west);
\end{tikzpicture}