...
from $R_{t=1}$.
Note that the \Call{Pivot}{$b_i, b_j, b_k, R_1$} call inside algorithm~\ref{algorithm:triangleIdentification}
...
%The function \Call{Common}{$R^{ab}, R^{ac}, E$} in algorithm~\ref{algorithm:pyramidHelpers} also specifies an $E$
...
\begin{table*}[ht]
    ...
    Each method's image features, reduction process, and alignment process is displayed.
